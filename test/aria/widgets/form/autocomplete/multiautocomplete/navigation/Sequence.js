...
/**
 * A sequence is a list of tasks.
 *
 * It can automatically be run itself as a task when it is added to another sequence.
 *
 * Otherwise it can be run as is, with a classical callback definition to be invoked when the sequence has finished.
 */
Aria.classDefinition({
    $classpath : "test.aria.widgets.form.autocomplete.multiautocomplete.navigation.Sequence",
    ...
    $prototype: {
         ...
         * Adds a child task.
         *
         * If it's not already one, builds it.
         *
        ...
        addTask : function(input) {
            ...
            var task;
            if (!aria.utils.Type.isInstanceOf(input, "test.aria.widgets.form.autocomplete.multiautocomplete.navigation.Task")) {
                task = new test.aria.widgets.form.autocomplete.multiautocomplete.navigation.Task(input);
            } else {
                task = input;
            }
        ...
        /**
         * Adds a child sequence, wrapped as a new task.
         *
         * If it's not already a sequence, builds it.
         *
         * Uses <code>addTask<code> to add the wrapping task.
         *
        ...
        addSequence : function(input) {
            ...
            var sequence;
            if (!aria.utils.Type.isInstanceOf(input, "test.aria.widgets.form.autocomplete.multiautocomplete.navigation.Sequence")) {
                sequence = new test.aria.widgets.form.autocomplete.multiautocomplete.navigation.Sequence(input);
            } else {
                sequence = input;
            }
        ...
        /**
         * Function to run the sequence when the latter is itself wrapped in a task.
         *
         * The difference is what to do when the sequence has finished: notify the end of the wrapping task.
         *
         * Uses <code>run</code> behind.
         *
         ...
         *
         * @param[in] {Function} onEnd The callback to be called when the sequence has completely finished. Defaults to a non-op.
         * @param[in] scope The scope of the callback. Defaults to <code>this</code>.
         * @param[in] arg The argument to be passed to the callback. Only one argument is supported, so if you need multiple manage yourself the use of an array or an object.
         */
