			...
			storage.$on({
				change : {
					fn : function (evt) {
						...
						this.assertEquals(aria.utils.Object.keys(evt.newValue || {}).length, expected.newValues, "Event " + counter + " has wrong newValue");
						this.assertEquals(aria.utils.Object.keys(evt.oldValue || {}).length, expected.oldValues, "Event " + counter + " has wrong oldValue");
					},
					scope : this
				}
			});

			var meta = {one : 1, two : 1};
			expected = {
				newValues : 2,
				oldValues : 0
			};
