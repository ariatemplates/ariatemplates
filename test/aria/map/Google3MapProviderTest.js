...
Aria.classDefinition({
	...
	$prototype : {
        ...
        _testUrlBuilder : function (_, args) {
            var url;

            url = this.provider._getFullUrl();
            this.assertEquals(url, "http://maps.googleapis.com/maps/api/js?v=3&sensor=false&callback=__googleMapLoaded",
                    "The url is not the expected one (case 1)");

            this.provider.credentials = "AAA";
            url = this.provider._getFullUrl();
            this.assertEquals(url, "http://maps.googleapis.com/maps/api/js?v=3&sensor=false&callback=__googleMapLoaded&key=AAA",
                    "The url is not the expected one (case 2)");

            this.provider.credentials = "gme-AAA";
            url = this.provider._getFullUrl();
            this.assertEquals(url, "http://maps.googleapis.com/maps/api/js?v=3&sensor=false&callback=__googleMapLoaded&client=gme-AAA",
                    "The url is not the expected one (case 2)");
