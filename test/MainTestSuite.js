...
Aria.classDefinition({
    ...
    $prototype : {
        ...
        testCalendarPageNavigation1 : function () {
            this._initCalendarCtrl({
                settings : {
                    startDate : new Date(2023, 1, 1),
                    minValue : new Date(2023, 1, 1),
                    maxValue : new Date(2023, 1, 28),
                    numberOfUnits : 1,
                    displayUnit : "M"
                }
            }, {
                fn : this._checkCalendarPageNavigation1,
                scope : this
            });
        },
        _checkCalendarPageNavigation1 : function () {
            try {
                var calendar = this._calendarCtrl.getData().calendar;
                console.log("Start Date: " + calendar.startDate);
                console.log("End Date: " + calendar.endDate);
                this.assertFalse(calendar.previousPageEnabled);
                this.assertFalse(calendar.nextPageEnabled);
            } catch (ex) {
        ...
        testCalendarPageNavigation2 : function () {
            this._initCalendarCtrl({
                settings : {
                    startDate : new Date(2023, 1, 1),
                    minValue : new Date(2022, 12, 31),
                    maxValue : new Date(2023, 2, 1),
                    numberOfUnits : 1,
                    displayUnit : "M"
                }
            }, {
                fn : this._checkCalendarPageNavigation2,
                scope : this
            });
        },
        _checkCalendarPageNavigation2 : function () {
            try {
                var calendar = this._calendarCtrl.getData().calendar;
                console.log("Start Date: " + calendar.startDate);
                console.log("End Date: " + calendar.endDate);
                this.assertTrue(calendar.previousPageEnabled);
                this.assertTrue(calendar.nextPageEnabled);
            } catch (ex) {
                this.handleAsyncTestError(ex, false);
            } finally {
                this._disposeCalendar();
