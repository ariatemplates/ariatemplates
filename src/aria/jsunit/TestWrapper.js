...
Aria.classDefinition({
	...
	$prototype : {
		...
		_atLoadedInFrame : function (res) {
			...
			// Load aria.widgets.AriaSkinInterface first, because most widgets use it to specify the classpath of their
			// CSS dependencies, so it must be there before evaluating the JS file)
			Aria.load({
				classes : ["aria.widgets.AriaSkinInterface"],
				oncomplete : {
					fn : this._ariaSkinInterfaceLoaded,
					scope : this
				},
				onerror : {
		...
		/**
		 * Called once aria.widgets.AriaSkinInterface is loaded, loads the test itself.
		 */
		_ariaSkinInterfaceLoaded : function () {
